...
#region Quick Reference Guide
        ...
        void DrawPlaybackTool(AudioFileObject myScript)
        {
            ...
            if (showPlaybackTool && myScript.GetFileCount() > 0)
            {
        ...
        public void CheckIfNameChanged()
        {
            ...
            if (AudioManager.instance)
            {
                if (AudioManager.instance.GetSceneSoundEnum() != null)
                {
        ...
        public Texture2D PaintWaveformSpectrum(AudioClip audio, int width, int height, Color col)
        {
            ...
            int leftSide = (int)(leftValue * samples.Length);
            int rightSide = (int)(rightValue * samples.Length);

            float zoomLevel = AudioPlaybackToolEditor.scrollZoom / AudioPlaybackToolEditor.MAX_SCROLL_ZOOM;
            int packSize = (int)((int)samples.Length / (int)width * (float)zoomLevel) + 1;
                ...
                case FadeMode.FadeIn: // Paint the fade-in area
                    {
                        // Scope lol
                        {
                            ...
                            float clipStartRelative = JSAMExtensions.InverseLerpUnclamped(leftValue, rightValue, myScript.clipStartPosition);
                            ...
                            // Get the length of the clip start section to push the entire fade graphic forward
                            int clipStartOffset = (int)(clipStartRelative * width);

                            // Offset the lerp value by how much the left side bar is obscuring the start of the fade
                            int offset = (int)(width * Mathf.Abs(fadeStartRelative));

                            // Clamp the limit in case fadeWidth exceeds the right side of the bar
                            for (int x = 0; x + offset < Mathf.Clamp(fadeWidth + clipStartOffset, 0, width) + offset; x++)
                            {
                                ...
                                // amountToPaint is the amount being painted on the vertical
                                int xOffset = x - clipStartOffset;
                                float lerpValue = (float)(xOffset + offset) / (fadeWidth + offset);
                            ...
                            for (int x = (int)Mathf.Clamp(fadeWidth + clipStartOffset, 0, width); x < width; x++)
                            {
                                for (int y = 0; y < height; y++)
                                {
                                    tex.SetPixel(x, y, lightShade);
                ...
                case FadeMode.FadeOut:
                    {
                        ...
                        int waveStart = (int)(width * JSAMExtensions.InverseLerpUnclamped(leftValue, rightValue, 1 - fadeOutDuration - myScript.clipEndPosition));

                        // Fill the area up to the start point of the fade (offset by clip end point) with grey
                        for (int x = 0; x < waveStart; x++)
                        {
                            for (int y = 0; y < height; y++)
                            {
                                tex.SetPixel(x, y, lightShade);
                        ...
                        float clipEndRelative = JSAMExtensions.InverseLerpUnclamped(leftValue, rightValue, myScript.clipEndPosition);

                        float fadeWidth = width * Mathf.Abs(fadeEndRelative - fadeStartRelative);

                        // x Starts where we previously left off
                        for (int x = waveStart; x < width; x++)
                        {
                            float lerpValue = (float)(x - waveStart) / (fadeWidth);
